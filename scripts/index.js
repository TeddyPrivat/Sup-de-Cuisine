...
function addTag(tagText) {
  ...
  if (
    [...tagsContainer.children].some(
      (tag) => tag.textContent.trim() === tagText
    )
  )return;
...
export function filterList(liste, searchInput, classe) {
...
function populateUstensilsList(ustensilArray) {
  ...
  searchInput.className = "search-bar";
  ustensilArray.sort((a, b) => a.localeCompare(b));
...
getData().then((recettes) => {
  ...
  const dropdownButtonAppliances = document.querySelector(
    ".dropdown_button_appliances"
  );
  const dropdownButtonUstensils = document.querySelector(
    ".dropdown_button_ustensils"
  );
  const dropdownButtonIngredients = document.querySelector(
    ".dropdown_button_ingredients"
  );
  ...
  dropdownButtonAppliances.addEventListener("click", () => {
    ...
    appliancesList.style.display =
      appliancesList.style.display === "none" ? "block" : "none";
  });

  dropdownButtonUstensils.addEventListener("click", () => {
    ustensilsList.style.display =
      ustensilsList.style.display === "none" ? "block" : "none";
  });
...
function displayRecettes(recettes) {
  ...
  if (recettes.length === 0) {
    ...
    noResultMessage.textContent =
      ...
      searchInput.value +
      " essayez plutôt 'tarte aux pommes'";
...
export function filterRecettes(recettes, searchTerm) {
  return recettes.filter((recette) => {
    ...
    const tagsMatch = tagUsedFilters.every((tag) => {
      ...
      const isIngredient = recette.ingredients.some(
        (ingredient) => ingredient.ingredient.toLowerCase() === tagLower
      );
      const isAppliance =
        recette.appliance && recette.appliance.toLowerCase() === tagLower;
      const isUstensil = recette.ustensils.some(
        (ustensil) => ustensil.toLowerCase() === tagLower
      );
